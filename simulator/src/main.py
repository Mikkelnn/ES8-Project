...
from custom_types import Area, Severity, SimState
...
class GUI(QMainWindow):
	def collect_input_state(self):
		...
		return {"until_time": until_time, "run_duration": run_duration, "severity": severity, "areas": areas}
	...
	def refresh_log_display(self):
		...
		if self._sim_state == SimState.RUNNING and self._target_tick is not None and self._latest_tick >= self._target_tick:
			self.pause_engine()
	...
	def update_est_time_label(self):
		...
		if until_time and hasattr(self, "_target_tick") and self._target_tick is not None:
			...
			self.est_time_label.setText(f"ETA: {eta_str} (TPS: {self._ewma_tps:.1f}) [{progress_bar}] {progress_pct}%")
		else:
