...
class PingPongProtocol(IModule):
    ...
    def tick(self, current_global_tick: int) -> float:    
        ...
        current_tranceiver_states = self.local_event_queue.get_current_events_by_type(LocalEventTypes.TRANCEIVER_STATUS)[0] # Always populated by tranceiver service before this protocol is ticked, so we can be sure to have it.
        ...
        if self.node_id == 1 and current_global_tick == 1:
            ...
            print(f"Node {self.node_id} sent a message at global tick {current_global_tick}...")
        ...
        match current_tranceiver_states[MediumTypes.LORA_D2D]:
            ...
            case TranceiverState.RECEIVING:
                if curretnt_receptions:
                    ...
                    self.local_event_queue.add_event_to_next_tick(type=LocalEventTypes.TRANCEIVER_TRANSMIT_DATA, sub_type=MediumTypes.LORA_D2D, data=[1]) # The content of the message does not matter in this protocol, so we just send a list with one element.
                    print(f"Node {self.node_id} received a message at global tick {current_global_tick}, sending response...")
