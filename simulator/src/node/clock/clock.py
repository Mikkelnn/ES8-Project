...
class Clock(IModule):
    ...
    def tick(self, current_global_tick: int) -> float | None:
        ...
        local_clock_info = LocalClockInfo(
            ...
            timer_1_remaining=max(0, self.timer_1_end_local_time - local_time) if self.timer_1_end_local_time is not None else None,
            timer_2_remaining=max(0, self.timer_2_end_local_time - local_time) if self.timer_2_end_local_time is not None else None
        )
