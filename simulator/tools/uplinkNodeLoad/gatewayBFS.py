...
graph = {int(nid): [int(nb) for nb in n["neighbours"]] for nid, n in nodes_raw.items()}
...
def main():
	...
	# 2. run global BFS
	visited, gateway_total, per_init_count, per_init_max_hops = global_bfs(graph, gateway_initials)
