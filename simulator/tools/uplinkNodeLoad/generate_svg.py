...
def generate_svg(
	...
	for rid in road_ids:
		...
		for nid, n in nodes.items():
			if n["road_id"] != rid:
				continue
			x, y = n["point"]
			node_col = col if int(nid) in visited else "#aa0000"
			dots.append(f'<circle cx="{x}" cy="{y}" r="{R_ROAD}" fill="{node_col}" data-nodes="{nid}"/>')
		if dots:
			L.append(f'<g opacity="0.92" filter="url(#rg_{rid})">')
			L.extend(dots)
			L.append("</g>")
	...
	if gateway_points:
		gw_color = "#22d3ee"
		r = 300 / meta["m_per_svg_x"]
		for _, x, y in gateway_points:
			L.append('<g filter="url(#gglow)">')
			L.append(f'  <circle cx="{x}" cy="{y}" r="{r}" fill="{gw_color}aa"/>')
