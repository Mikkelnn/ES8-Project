...
def dist_m(a, b):
	...
	return math.hypot((b[0] - a[0]) * M_PER_SVG_X, (b[1] - a[1]) * M_PER_SVG_Y)
		...
		for i in range(len(chains)):
			...
			while ext:
				...
				for j in range(len(chains)):
					...
					o = chains[j]
					t, h = chain[-1], chain[0]
					if dist_svg(t, o[0]) <= tol:
						chain = chain + o[1:]
					elif dist_svg(t, o[-1]) <= tol:
						chain = chain + list(reversed(o))[1:]
					elif dist_svg(h, o[-1]) <= tol:
						chain = o + chain[1:]
					elif dist_svg(h, o[0]) <= tol:
						chain = list(reversed(o)) + chain[1:]
					else:
						continue
					used[j] = True
					merged = True
...
def remove_parallel_chains(chains, parallel_tol_m=2.0):  # was 5.0
		...
		for p in pts:
			...
			for i in range(len(reference) - 1):
				...
				if lsq == 0:
					...
					t = max(0.0, min(1.0, ((p[0] - a[0]) * dx + (p[1] - a[1]) * dy) / lsq))
	...
	for candidate in sorted_chains[1:]:
		is_parallel = any(avg_dist_to_chain(candidate, ref) < parallel_tol_m for ref in kept)
		if not is_parallel:
...
def generate(input_path=INPUT_FILE, output_path=OUTPUT_FILE, merge_radius_m=MERGE_RADIUS_M):
	...
	for rid, chains in tqdm.tqdm(road_chains.items(), "Placing nodes on roads"):
		...
		for ci, chain in enumerate(chains):
			...
			for _, mid, _ in ints_here:
				arc, snapped, _ = project_onto_chain(merged_ints[mid]["point"], working)
				splits.append((arc, mid, snapped))
			splits.sort(key=lambda x: x[0])

			total_arc = sum(dist_svg(working[i], working[i + 1]) for i in range(len(working) - 1))
			breakpoints = [(0.0, None, working[0])] + [(a, m, s) for a, m, s in splits] + [(total_arc, None, working[-1])]
			seg_node_seqs = []
	...
	orphaned = [nid for nid, nd in nodes.items() if nd["intersection_id"] is None and len(nd["neighbours"]) == 0]
	for nid in orphaned:
		del nodes[nid]

	if removed_edges or orphaned:
		print(f"  Post-process: removed {removed_edges // 2} spurious edges, {len(orphaned)} orphaned nodes")
	...
	print(f"    mean={sp_mean:.4f}m  median={sp_median:.4f}m  min={sp_min:.4f}m  max={sp_max:.4f}m")
	print(f"\n  Written -> {out_path}")
	print(f"\n{'=' * 65}  Done.\n")
