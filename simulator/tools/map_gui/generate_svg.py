...
def generate_svg(
	...
	L.append(f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="{vbx:.4f} {vby:.4f} {vbw:.4f} {vbh:.4f}" width="{SVG_W}" height="{SVG_H}" style="background:#080d14;">')

	# Defs
	L.append("<defs>")
	for rid in road_ids:
		L.append(f'  <filter id="rg_{rid}" x="-60%" y="-60%" width="220%" height="220%"><feGaussianBlur stdDeviation="0.018" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>')
	L.append('  <filter id="iglow" x="-150%" y="-150%" width="400%" height="400%"><feGaussianBlur stdDeviation="0.07" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>')
	L.append('  <filter id="mglow" x="-150%" y="-150%" width="400%" height="400%"><feGaussianBlur stdDeviation="0.10" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>')
	L.append('  <filter id="gglow" x="-150%" y="-150%" width="400%" height="400%"><feGaussianBlur stdDeviation="0.07" result="b"/><feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge></filter>')
	L.append("</defs>")
	...
	for nid, n in int_nodes.items():
		...
		L.append(f'  <circle cx="{x}" cy="{y}" r="{R_INT}" fill="none" stroke="{rc}" stroke-width="{sw}"/>')
	...
	if gateway_points:
		...
		for _, x, y in gateway_points:
			...
			L.append(f'  <circle cx="{x}" cy="{y}" r="{R_INT}" fill="none" stroke="{gw_color}" stroke-width="0.032"/>')
	...
	L.append(f'<rect x="{lx - 0.12:.3f}" y="{ly - 0.22:.3f}" width="{bw}" height="{box_h:.3f}" fill="#080d14" fill-opacity="0.80" rx="0.10"/>')
	L.append(f'<text x="{lx + 0.05:.3f}" y="{ly:.3f}" font-size="{ts * 1.05}" fill="#ffffff" font-weight="bold" font-family="monospace">Node map  (merge r={merge_r} m, interval={interval} m)</text>')
	ly += rh * 1.1

	for rid in road_ids:
		col = ROAD_COLORS.get(rid, "#aaa")
		L.append(f'<circle cx="{lx + dr:.3f}" cy="{ly - dr * 0.6:.3f}" r="{dr}" fill="{col}"/>')
		L.append(f'<text x="{lx + dr * 2.8:.3f}" y="{ly:.3f}" font-size="{ts * 0.82}" fill="{col}" font-family="monospace">road {rid}</text>')
		ly += rh

	L.append(f'<line x1="{lx:.3f}" y1="{ly - rh * 0.3:.3f}" x2="{lx + bw - 0.3:.3f}" y2="{ly - rh * 0.3:.3f}" stroke="#ffffff22" stroke-width="0.015"/>')
	L.append(f'<circle cx="{lx + dr:.3f}" cy="{ly - dr * 0.6:.3f}" r="{dr * 1.6}" fill="none" stroke="#ffffff" stroke-width="0.022"/>')
	L.append(f'<circle cx="{lx + dr:.3f}" cy="{ly - dr * 0.6:.3f}" r="{dr * 0.55}" fill="#ffffff"/>')
	L.append(f'<text x="{lx + dr * 2.8:.3f}" y="{ly:.3f}" font-size="{ts * 0.82}" fill="#ffffff" font-family="monospace">intersection node</text>')
	ly += rh
	L.append(f'<circle cx="{lx + dr:.3f}" cy="{ly - dr * 0.6:.3f}" r="{dr * 1.6}" fill="none" stroke="#fbbf24" stroke-width="0.028"/>')
	L.append(f'<circle cx="{lx + dr:.3f}" cy="{ly - dr * 0.6:.3f}" r="{dr * 0.55}" fill="#fbbf24"/>')
	L.append(f'<text x="{lx + dr * 2.8:.3f}" y="{ly:.3f}" font-size="{ts * 0.82}" fill="#fbbf24" font-family="monospace">merged intersection</text>')
	ly += rh
	L.append(f'<circle cx="{lx + dr:.3f}" cy="{ly - dr * 0.6:.3f}" r="{dr * 1.35}" fill="#22d3ee0d" stroke="#22d3ee" stroke-width="0.022"/>')
	L.append(f'<circle cx="{lx + dr:.3f}" cy="{ly - dr * 0.6:.3f}" r="{dr * 0.55}" fill="#22d3ee"/>')
	L.append(f'<text x="{lx + dr * 2.8:.3f}" y="{ly:.3f}" font-size="{ts * 0.82}" fill="#22d3ee" font-family="monospace">gateway point</text>')
	L.append("</g>")
	L.append("</svg>")
	...
	n_int = len(int_nodes)
	n_merged = sum(1 for n in int_nodes.values() if len(n.get("source_intersection_ids") or []) > 1)
	n_gateways = len(gateway_points)
	print(f"SVG written -> {out}  ({len(svg) // 1024} KB, {SVG_W}x{SVG_H}px)  {len(nodes)} nodes  |  {n_int} intersection nodes  ({n_merged} merged)  |  {n_gateways} gateways")
