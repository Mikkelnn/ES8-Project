...
def find_road_intersections(gdf):
	...
	for i, j in tqdm.tqdm(zip(left_idx, right_idx), "Intersection combinations", total=min(len(left_idx), len(right_idx))):
		...
		for pt in pts:
			intersection_points.append({"geometry": pt, "road_a_name": name_a, "road_a_osm_id": osm_a, "road_a_fclass": fclass_a, "road_b_name": name_b, "road_b_osm_id": osm_b, "road_b_fclass": fclass_b, "intersecting_roads": f"{name_a} ({fclass_a}, osm_id={osm_a}) x {name_b} ({fclass_b}, osm_id={osm_b})"})

	if not intersection_points:
		print("No intersections found.")
		return gpd.GeoDataFrame(columns=["geometry", "road_a_name", "road_a_osm_id", "road_a_fclass", "road_b_name", "road_b_osm_id", "road_b_fclass", "intersecting_roads"], geometry="geometry", crs="EPSG:3857")
...
def _build_projector(gdf: gpd.GeoDataFrame, svg_w: float = 1200, svg_h: float = 900, padding: float = 40.0):
	"""
	...
	metadata = {
		...
		"svg_dimensions": {"width": svg_w, "height": svg_h, "used_width": usable_w, "used_height": usable_h, "padding": padding, "m_per_svg_x": m_per_svg_x, "m_per_svg_y": m_per_svg_y},
...
def _geom_to_path_details(geom, project) -> list:
	...
	if isinstance(geom, MultiLineString):
		return [_linestring_to_svg_path(line.coords, project) for line in geom.geoms]
	return []
...
def _geom_to_point(geom, project) -> list:
	...
	if isinstance(geom, MultiPoint):
		return [{"cx": project(pt.x, pt.y)[0], "cy": project(pt.x, pt.y)[1]} for pt in geom.geoms]
	return []
...
def gdf_to_road_intersection_json(gdf: gpd.GeoDataFrame, svg_width: int = 1200, svg_height: int = 900, padding: float = 40.0) -> dict:
	...
	for _, row in gdf.iterrows():
		...
		# ── Intersections (Point / MultiPoint) ────────────────────────────── #
			...
			road_osm_ids = ([str(row["road_a_osm_id"])] if not _is_null(row.get("road_a_osm_id")) else []) + ([str(row["road_b_osm_id"])] if not _is_null(row.get("road_b_osm_id")) else [])
...
def result_to_js(result: dict, output_path: str = "processedRoads_test.js", variable_name: str = "RAW") -> dict:
	"""
	...
	payload = {"metadata": metadata_in, "roads": roads_out, "intersections": intersections_out, "roads_bb": roads_bb_out}
